...
<div class="main-content">
          ...
          <div class="card">
            <div class="card-body">
              ...
              @if ($proposal->reviews->last()->acc == 1)
                <a type="button" class="btn btn-danger" href="{{ route('admin.proposal.download', ['file' => $proposal->file]) }}"><i class="fas fa-file-pdf"></i> Proposal ACC</a>
              @else
  ...
  <form action="{{ route('usulan.download.berita') }}" id="form-berita" method="get" class="d-none">
    @csrf
    <input type="text" name="id" value="{{ $proposal->id }}">
  </form>

  <form action="{{ route('usulan.download.form') }}" id="form-penilaian" method="get" class="d-none">
    @csrf
    <input type="text" name="id" value="{{ $proposal->id }}">
  </form>
</div>
